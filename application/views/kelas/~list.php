...
<script type="application/javascript">
	$(document).ready(function(){
		...
		$.each($('img[dee-picture]'), function(i, elm){
			var t = $('<div class="price-tag orange bold"><div>');
			var price = $(elm).data('price');
			price = price.replace('#', '<br />');
			t.html("<span class='white text-16'>DISKON</span><br/>"+price);
			$(elm).before(t);
			console.log(elm);
		});
		$.each($('img[dee-discount]'), function(i, elm){
			...
			$('<div class="discount-tag"></div>')
					.append($("<img src='<?php echo base_url();?>images/discount.png' width='100px'/>"))
			);
		});
